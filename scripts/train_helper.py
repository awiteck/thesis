...
def run_training_and_validation(
    ...
    indices = utils.get_indices_entire_sequence(
        ...
        # step_size=int((config["enc_seq_len"] / 3)),
    )
    training_indices, val_indices, test_indices = random_split(
        indices, [0.8, 0.15, 0.05]
    )
    # random.shuffle(indices)
    # split_index = int(len(indices) * config["test_size"])
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for batch_idx, batch in enumerate(training_data):
            ...
            if batch_idx % 250 == 249:
                last_loss = epoch_loss / 250  # loss per batch
                epoch_loss = 0.0
        ...
        print(
            f"Epoch {epoch+1}/{config['num_epochs']}, Last Batch Loss: {last_loss:.4f}, Validation Loss: {avg_val_loss:.4f}"
        )
    ...
    vals = run_validation(model, test_data, config["output_sequence_length"], device)
    if write_example_predictions:
        # Keys of interest
        keys_of_interest = ["source_readings", "expected", "predicted"]
...
def greedy_decode(
    model, source, source_mask, max_len, device, input_dim=1, contextual_vars=None
):
    """
    contextual_vars is a 2D tensor of shape (max_len, input_dim-1)
    """
    assert not (
        input_dim > 1 and contextual_vars is None
    ), "For inputs of more than one feature, contextual vars must be given beforehand."
    ...
    while True:
        ...
        next_val = next_val[0, -1, 0]

        next_vals = torch.empty(1, 1, input_dim).type_as(source).to(device)
        next_vals[0, 0, 0] = next_val.item()
        if input_dim > 1:
            next_vals[0, 0, 1:] = contextual_vars[i, :]
...
def run_validation(
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(validation_data):
            ...
            context = (
                future_data[:, :, 1:].squeeze(0) if input_dim > 1 else None
            )  # (output length, input_dim - 1)
            ...
            # print(f"target_reading.size(): {label[0].size()}")

            # source_reading = encoder_input[0]

            target_reading = label[0]
...
def get_model(config):
    model = build_transformer(
        ...
        offset=1 + len(config["continuous_vars"]),
    )
    return model
